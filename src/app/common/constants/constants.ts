export class COMMON_CONSTANTS {
	public static STATUS_OK = 200;
	public static WHATSAPP_URL = 'https://api.whatsapp.com/send?phone=593992826825';
	public static WHATSAPP_ICON_URL = 'https://i.imgur.com/F3Nb4kf.png';
	public static WHATSAPP_ICON_HTML = `<img src="${COMMON_CONSTANTS.WHATSAPP_ICON_URL}">`
	public static CONTENT_TYPE = 'application/x-www-form-urlencoded';
	public static PRODUCT_CATALOG_DATA = [
		{
			productImage: 'https://target.scene7.com/is/image/Target/2019_4Wk3_HHE_NaturalCleaning_CatNav_1_All-PurposeCleaners170634-190411_1555040147518?wid=328&hei=328&qlt=80&fmt=pjpeg',
			productTitle: 'The Product',
			productDescription: 'Lorem ipsum sit amet, et audem et omnia sunt',
			productPrice: '99.99'
		},
		{
			productImage: 'https://target.scene7.com/is/image/Target/2019_4Wk3_HHE_NaturalCleaning_CatNav_1_All-PurposeCleaners170634-190411_1555040147518?wid=328&hei=328&qlt=80&fmt=pjpeg',
			productTitle: 'The Product',
			productDescription: 'Lorem ipsum sit amet, et audem et omnia sunt',
			productPrice: '99.99'
		},
		{
			productImage: 'https://target.scene7.com/is/image/Target/2019_4Wk3_HHE_NaturalCleaning_CatNav_1_All-PurposeCleaners170634-190411_1555040147518?wid=328&hei=328&qlt=80&fmt=pjpeg',
			productTitle: 'The Product',
			productDescription: 'Lorem ipsum sit amet, et audem et omnia sunt',
			productPrice: '99.99'
		},
		{
			productImage: 'https://target.scene7.com/is/image/Target/2019_4Wk3_HHE_NaturalCleaning_CatNav_1_All-PurposeCleaners170634-190411_1555040147518?wid=328&hei=328&qlt=80&fmt=pjpeg',
			productTitle: 'The Product',
			productDescription: 'Lorem ipsum sit amet, et audem et omnia sunt',
			productPrice: '99.99'
		},
		{
			productImage: 'https://target.scene7.com/is/image/Target/2019_4Wk3_HHE_NaturalCleaning_CatNav_1_All-PurposeCleaners170634-190411_1555040147518?wid=328&hei=328&qlt=80&fmt=pjpeg',
			productTitle: 'The Product',
			productDescription: 'Lorem ipsum sit amet, et audem et omnia sunt',
			productPrice: '99.99'
		},
	];
}

export class TEXT_CONSTANTS {
	public static GENERAL_INFO_TITLE = "Información general del Cannabidiol";
	public static GENERAL_INFO_MESSAGE = "<img style='width: 100%; padding: 0 10% 0;' src='https://i.imgur.com/9e068Pf.jpg' > </br></br> Seguro has escuchado hablar del CBD o cannabidiol. <br/><br/>Es uno de los compuestos medicinales más poderosos de la planta del cannabis, que cada día cobra más fama como agente terapéutico ante diferentes enfermedades y padecimientos. Y seguramente también te preguntas para qué sirve. ¿Por qué tanto alboroto por este cannabinoide? Te hablamos un poquito de sus propiedades terapéuticas y algunos productos que ya existen en el mundo, que lo contienen<br/><br/><strong>¿Qué es el CBD?</strong><br/>El cannabidiol es uno de los cannabinoides más importantes presentes en la planta de cannabis. Estos compuestos son los productos químicos que activan los receptores cannabinoides en el cerebro y en el cuerpo de todos los mamíferos.<br/>El cannabidol posee muchos efectos medicinales, con la ventaja de que no es psicoactivo, es decir, no te pone como sí lo hace el THC, otro de los más de cien cannabinoides de la planta. Incluso, el CDB ayuda a nivelar los efectos del THC en cepas con una presencia alta de este último, reduciendo los ataques de ansiedad, las palpitaciones y otros efectos adversos que puede llegar a causar.</br>En noviembre pasado, la Organización Mundial de la Salud (OMS) reconoció que el cannabidiol no es adictivo ni psicoactivo: «La OMS, órgano de las Naciones Unidas, determinó que el cannabidiol (CBD), molécula no psicoactiva de la planta Cannabis sativa, no es una sustancia peligrosa, por el contrario, cuenta con un alto potencial terapéutico.<br/><br/><strong>¿En qué ayuda el CBD?</strong><br/>Los estudios clínicos y científicos demuestran que tiene potencial terapéutico para muchas patologías, que incluyen:<br/><ul><li>Dolor crónico</li><li>Cáncer</li><li>Ansiedad</li><li>Diabetes</li><li>Epilepsia</li><li>Artritis y reumas</li><li>Trastorno de Estrés Post Traumático</li><li>Insomnio</li><li>Alcoholismo</li><li>Enfermedades cardio-vasculares</li><li>Infecciones resistentes a los antibióticos</li><li>Problemas neurológico</li><li>Insomnio</li></ul></br></br>Además, tiene propiedades antioxidantes y es rico en ácidos grasos omega 3 y 6. </br> Un producto elaborado con la planta completa de cannabis y no con CBD sintético sino natural, siempre será la mejor opción natural debido al efecto sinérgico de todos los componentes del cannabis, incluyendo los terpenos y los flavonoides, que actúan en efecto séquito.</br> Hay padecimientos, como el cáncer, para los cuales el THC es mucho más efectivo. Sin embargo, el CBD por sí mismo puede ser una gran ayuda en varios casos, y ya podemos encontrarlo en una gran diversidad de productos que lo contienen, solo o acompañado de THC y otros cannabinoides.";
	public static CONTACT_TITLE = `INFORMACIÓN DEL CONTACTO ${COMMON_CONSTANTS.WHATSAPP_ICON_HTML}`;
	public static CONTACT_MESSAGE = `<strong>Telefonos:</strong><ul><li><a href='tel:0995320625'>0995320625</a> - <a href='https://api.whatsapp.com/send?phone=593995320625'>WhatsApp</a></li><br><li><a href='tel:0992826825'>0992826825</a> - <a href='https://api.whatsapp.com/send?phone=593992826825'>WhatsApp</a></li></ul>`;
	public static POLICY_TITLE = "POLÍTICA DE ENVÍOS";
	public static POLICY_MESSAGE = "Trabajamos por medio de TRAMACO y Servientrega con envíos a todas las provincias del Ecuador, incluido Galápagos, con deposito previo y 100% seguro.";
	public static ABOUT_TITLE = "SOBRE NOSOTROS";
	public static ABOUT_MESSAGE = "Nuestro CBD Lazaros naturales está considerado entre las 12 mejores marcas de CBD de los Estados Unidos, gozando de gran reputación. <br/><br/>Desde nuestras prácticas agrícolas y procesos de producción, hasta cómo se empaquetan nuestros productos, siempre buscamos formas de hacer negocios ambiental y socialmente responsables. Por ejemplo, cada pedido se embala en nuestras instalaciones con materiales fabricados a partir de residuos posconsumo. Nuestro objetivo actual es ser neutrales en carbono para 2030, con el objetivo a largo plazo de eliminar por completo todos los desechos no biodegradables y las emisiones de carbono para 2040.<br/><br/>Nuestra misión siempre ha sido fabricar productos que mejoren la vida de las personas, así como el mundo que nos rodea, operando de manera justa, equitativa y ambientalmente sostenible. Nuestros esfuerzos con este fin nos han llevado a recibir las certificaciones USDA Organic , Leaping Bunny y B Corp. Lazarus Naturals se compromete a utilizar los negocios como una fuerza para el bien y nunca poner las ganancias por encima del propósito. No se trata de ser el mejor del mundo, sino de hacer lo mejor para el mundo.";
	public static TRANSFER_TITLE = "DATOS DE TRANSFERENCIA";
	public static TRANSFER_TEXT = "<div class='textwidget'><p><strong>Contacto:</strong><br><br><a href='tel:0995320625'>0995320625</a>  <br><a href='tel:0992826825'>0992826825</a><br></p><p><strong>Pedidos </strong><br>Recibimos pedidos las 24 horas del día.<br>Contáctanos y te enviaremos los productos inmedatamente. <br>Envios por Servientrega y TRAMACO a todo el Ecuador.<br></p><p><strong>Cuentas Bancarias </strong><br>Banco del Pichincha (Ahorros): 2200728200<br><strong>Nombre: Enrique Cardoso</strong> <br><strong>CI: 0102420460</strong></p></div>";
}

export class CART_CONSTANTS {
	public static GENERAL_INFO_TITLE = "Información general del Cannabidiol";
	public static PAYPHONE_BEAR = "mCcSY_famJdJ7ITebuxV6ubGU02liB8hLhB8y73n_qMqyLXWUNNrNq7W0jvBApuiWORNR3ZapZJ5n3rmWReozHXW4Rz2UgkQknZjgF15u0s6fLVjlQlreFZusL9xvd-5MoaVU20qIj-FkysnmR_j0Fa2TWcB_wYJGbH2Xk5GVQGGAeJJ-6GClnyy6Jh9VsjCvsVEtHkSfc0OhQNQFWNaAPrDYptlmM1PfFPLObmtJT4wxShOYed12yqUMZiI54gm7k1tpGrX7_ehaIMPc8SSFMeQ8qvO32OhGY0eVrvK3Urplv1a7NdiPgn0SztS6xgjjlxJbA";
}
